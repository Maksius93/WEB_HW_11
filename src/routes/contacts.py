...
async def create_contact(body: ContactsSchema, db: AsyncSession = Depends(get_db)):
    contact = await repository_contacts.create_contact(body, db)
    return contact


@router.put("/{contact_id}", response_model=ContactsResponse)
async def update_contact(body: ContactsUpdateSchema, contact_id: int = Path(ge=1), db: AsyncSession = Depends(get_db)):
